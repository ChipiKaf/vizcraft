...
controller.restart();    // restarts from progress = 0

// Subscribe to completion events (fires at end of each traversal)
const unsub = controller.onSignalComplete('flow', () => {
  console.log('flow signal completed one lap');
});
...
For fine-grained control — or when you're driving progress from your own animation
loop — use `patchSignals` directly on the `MountController`:
...
Call `controller.clearSignals()` to remove all manually-drawn signal dots.
