...
| `x`           | `number`             | auto      | Centre X. Optional — omit to use [auto-layout](/docs/how-to/auto-layout) |
| `y`           | `number`             | auto      | Centre Y. Optional — omit to use [auto-layout](/docs/how-to/auto-layout) |
...
## Structural & semantic spec features

Specs go far beyond flat nodes with pinned coordinates. See the dedicated
guides for the full declarative feature set:

- **[Containers, Groups & Zones](/docs/how-to/containers-groups-and-zones)** —
  `type: 'group'` frames that own and hug their children, `type: 'zone'`
  dashed regions, `collapsed` groups and `focus` mode.
- **[Auto-Layout](/docs/how-to/auto-layout)** — omit `x`/`y` and let deterministic
  `layered` / `grid` / `stack` engines place nodes (per view and per group).
- **[Edge Ports & Routing](/docs/how-to/ports-and-routing)** — `from: 'node.e'`
  side/named ports, clean container-wall exits, `routing: 'avoid'` obstacle
  avoidance and parallel-edge separation.
- **[Semantic Kinds, Legends & Notes](/docs/how-to/semantic-kinds-and-legends)** —
  typed edges/nodes via `kind`, a diagram-level `theme`, `legend: 'auto'`,
  view `title`/`subtitle`, and anchored `type: 'note'` annotations.
