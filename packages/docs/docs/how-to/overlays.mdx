...
export const basicSignalScene = basicSignalBuilder
  ...
  .overlay((o) =>
    o.add('signal', { from: 'a', to: 'b', progress: 0.5, magnitude: 0.7 }, { key: 'sig' })
  );
...
export const multiOverlayScene = multiOverlayBuilder
  ...
  .overlay((o) =>
    o
      ...
      .add('signal', { from: 'a', to: 'b', progress: 0.25, magnitude: 0.5 }, { key: 'sig', className: 'viz-signal' })
  );
...
export const overlayAnimScene = overlayAnimBuilder
  ...
  .node('b').at(400, 100).circle(18).label('B')
  .overlay((o) =>
    o.add('signal', { from: 'a', to: 'b', progress: 0, magnitude: 0.2 }, { key: 'sig' })
  );
...
builder
  ...
  .node('b').at(400, 100).rect(110, 78, 14).label('B', { dy: -52, fontWeight: 700 })
  .overlay((o) => {
    const r = 5, gap = 4, step = r * 2 + gap;
    const yOffsets = [-2 * step, -step, 0, step, 2 * step];
    o.group(
      { from: 'a', to: 'b', progress: 0, magnitude: 0.2 },
...
*Found a problem? [Open an issue](https://github.com/ChipiKaf/vizcraft/issues/new?labels=documentation&title=Docs:+Overlays) on GitHub.*
