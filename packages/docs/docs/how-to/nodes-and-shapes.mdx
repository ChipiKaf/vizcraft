...
export const entryPaddingScene = viz()
  .view(500, 280)
  .node('svc')
  .at(250, 140)
  .rect(240, 0, 6)
...
`}>
  ...
  <VizMount builder={entryPaddingScene} style={{ height: '280px', width: '100%' }} />
</CodePreview>
...
export const collapsedScene = viz()
  .view(500, 200)
  .node('expanded')
  .at(130, 100)
  .rect(200, 0, 6)
...
`}>
  ...
  <VizMount builder={collapsedScene} style={{ height: '200px', width: '100%' }} />
</CodePreview>
...
export const collapseToggleScene = viz()
  .view(600, 250)
  .node('cls1')
  .at(150, 125)
  .rect(200, 0, 6)
...
`}>
  ...
  <VizMount builder={collapseToggleScene} style={{ height: '250px', width: '100%' }} />
</CodePreview>
...
| `toggle`        | `(opts?: { animate?: number }) => void` | Helper to toggle collapse with optional animation |
...
See [CollapseIndicatorOptions](/docs/reference/types#collapseindicatoroptions) for all options.
...
export function CollapseAnchorDemo() {
  const [anchor, setAnchor] = useState('center');
  const builderRef = useRef(null);
  if (!builderRef.current) {
    builderRef.current = viz()
      .view(600, 300)
      .node('svc')
      .at(200, 150)
      .rect(200, 0, 6)
      .fill('#f5f5f5')
      .stroke('#333')
      .collapseAnchor('center')
      .compartment('name', (c) =>
        c
          .label('UserService')
          .height(36)
          .onClick((ctx) => ctx.toggle({ animate: 300 }))
      )
      .compartment('attrs', (c) =>
        c.label('- db: Database\n- cache: Cache', {
          fontSize: 12,
  ...
  useEffect(() => {
    builderRef.current.updateNode('svc', { collapseAnchor: anchor });
    builderRef.current.updateNode('db', { collapseAnchor: anchor });
  }, [anchor]);
  const btnStyle = (a) => ({
    padding: '4px 12px',
    borderRadius: 4,
    border: '1px solid #888',
    background: anchor === a ? '#333' : '#fff',
    color: anchor === a ? '#fff' : '#333',
    cursor: 'pointer',
    fontWeight: anchor === a ? 700 : 400,
  });
  return (
    <div>
      <div style={{ marginBottom: 8, display: 'flex', gap: 8 }}>
        {['top', 'center', 'bottom'].map((a) => (
          <button key={a} onClick={() => setAnchor(a)} style={btnStyle(a)}>
            {a}
          </button>
        ))}
      </div>
