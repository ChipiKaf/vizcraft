...
export const collapseToggleScene = viz()
  ...
  .stroke('#333')
  .compartment('name', (c) =>
    c.label('MyClass').height(36).onClick((ctx) => {
...
builder.node('cls')
.at(250, 100).rect(200, 0, 6).fill('#f5f5f5').stroke('#333')
.compartment('name', (c) =>
  c.label('MyClass').height(36).onClick((ctx) => {
    ctx.toggle({ animate: 200 }); // smooth 200ms animation
  })
)
.compartment('attrs', (c) =>
  c.label('- id: number\\n- name: string', { fontSize: 12, textAnchor: 'start' })
...
`}>
  ...
  <VizMount builder={collapseToggleScene} style={{ height: '200px', width: '100%' }} />
</CodePreview>
...
builder.node('cls', {
  ...
  collapsed: true,
  compartments: [
    { id: 'name', label: 'ClassName', height: 36, onClick: (ctx) => ctx.toggle() },
    { id: 'attrs', label: '- field: string' },
    { id: 'methods', label: '+ doWork()' },
  ],
});
...
builder.node('no-chevron')
  .at(100, 100).rect(160, 0)
...
// Custom colour
builder.node('red-chevron')
  .at(300, 100).rect(160, 0)
  .compartment('header', c => c.label('Title').onClick(ctx => ctx.toggle()))
  .compartment('body', c => c.label('Content'))
