...
### Subscribing from outside the builder

Fluent `.onClick()` only works when you own the builder call site. When the
scene comes from [`fromSpec`](/docs/how-to/from-spec) (JSON can't carry
function handlers) or when the diagram is **embedded** — an iframe, a
downstream component, a page that only receives the mounted SVG — the
[`MountController`](/docs/reference/types#mountcontroller) exposes
`onNodeClick(cb)` and `onEdgeClick(cb)`. Both fire for **every** node/edge,
even ones with no fluent `onClick`, and return an unsubscribe function.
...
const off = controller.onNodeClick(({ id, node, originalEvent }) => {
  console.log('clicked', id, node.label?.text);
  if (originalEvent.shiftKey) selectAdditional(id);
});

controller.onEdgeClick(({ id, edge }) => {
  console.log('clicked edge', id, edge.from, '→', edge.to);
});

// later: off();
```

The same event is also dispatched as a bubbling `CustomEvent` named
`vizcraft:node-click` (and `vizcraft:edge-click`) on the SVG root. That
matters for **cross-frame embeds**: a parent page can listen for clicks
without touching the child's runtime.

```ts
// The consumer only has the SVG root — e.g. an iframe or a web component:
svgRoot.addEventListener('vizcraft:node-click', (e) => {
  const { id, node, originalEvent } = e.detail;
  parent.postMessage({ type: 'node-click', id }, '*');
});
```

Every edge automatically gets a transparent 10px hit path so clicks are
easy to land — override with `edge.hitArea` for a bigger or smaller target.
...
const state = controller.getState(); // { zoom: number, pan: Vec2 }
...
### Persisting viewport across scene rebuilds

When a scene is rebuilt (e.g. in a React `useLayoutEffect`), the
`PanZoomController` is destroyed and the viewport resets. Use `getState()` to
capture the viewport before destroying, then pass the values back via
`initialZoom` and `initialPan`:

```typescript
// 1. Read from the outgoing controller
const prev = oldController.getState();
oldScene.destroy();

// 2. Mount the new scene with the saved viewport
const newController = newScene.mount(container, {
  panZoom: true,
...
`initialPan` is only used when `initialZoom` is a number (not `'fit'`).
