...
@keyframes vizPulse {
  ...
  50% { transform: scale(var(--viz-anim-scale, 1.35)); opacity: 0.35; stroke-width: 6; }
}
...
defaultCoreAnimationRegistry.registerNode('pulse', pulseAnim);
...
export const timelineAnim = timelineBuilder.animate((anim) => {
  anim.node('a');
  anim.to({ x: 220 }, { duration: 400 });
  anim.wait(250);
  anim.to({ x: 120 }, { duration: 400 });
  anim.at(0);
  anim.node('b');
  anim.to({ y: 170 }, { duration: 600 });
});
...
1. **Registry/CSS animations (legacy / lightweight)** — e.g. `.animate('flow')` adds CSS classes/vars.
2. **Data-only timeline animations (AnimationSpec)** — authored with a fluent callback and played via `builder.play()`.

Both approaches render efficiently by patching the mounted SVG in-place.

---

## 1) Registry/CSS animations (e.g. `flow`)

Registry animations are great when you want a simple, reusable effect that doesn’t need per-frame JS interpolation.
...
:::tip
VizCraft can inject registry/CSS animation styles directly into the mounted SVG via `builder.mount(container, { css })`.
If you prefer global styles (e.g. in Docusaurus), you can also put this CSS in `packages/docs/src/css/custom.css`.
:::
...
## 2) Data-only animations (`AnimationSpec`)
...
anim
.node('a')
.to({ x: 220 }, { duration: 400 })
.wait(250)
.to({ x: 120 }, { duration: 400 })
.at(0)
.node('b')
.to({ y: 170 }, { duration: 600 })
);
...
`}>
  ...
  <VizPlaybackControls builder={timelineScene} />
</CodePreview>
