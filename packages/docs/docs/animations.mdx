...
export const customPropAnim = customPropBuilder.animate((anim) => {
  anim.extendAdapter(extendCircleRadius);
  anim.node('a');
  anim.to({ r: 44 }, { duration: 900, easing: 'easeInOut' });
  anim.wait(250);
  anim.to({ r: 18 }, { duration: 900, easing: 'easeInOut' });
});
...
`AnimationSpec` can target **any string property** (not just the core ones), as long as the playback adapter knows how to read/write it.

You can register custom properties directly on the `anim` builder via `anim.extendAdapter(...)`. Here’s an example that animates a circle node’s radius via a custom prop named `r`.
