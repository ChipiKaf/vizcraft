...
- `style?: { fill?: string; stroke?: string; strokeWidth?: number; opacity?: number; strokeDasharray?: string; shadow?: { dx?: number; dy?: number; blur?: number; color?: string } }` — base styling. `strokeDasharray` accepts presets (`'dashed'`, `'dotted'`, `'dash-dot'`, `'solid'`) or any SVG dasharray string. `shadow` adds a drop shadow via an SVG filter (defaults: dx=2, dy=2, blur=4, color=rgba(0,0,0,0.2)).
