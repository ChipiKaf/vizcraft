...
- `style?: { stroke?: string; strokeWidth?: number; fill?: string; opacity?: number; strokeDasharray?: string }` — per-edge visual styling. `strokeDasharray` accepts presets (`'dashed'`, `'dotted'`, `'dash-dot'`, `'solid'`) or any SVG dasharray string. When set, inline SVG attributes override the CSS class defaults.
...
- `image?: NodeImage` — embedded image or SVG icon options.
...
Embedded image options for a node, useful for architecture blocks or custom icons.

```typescript
export interface NodeImage {
  href: string; // Image URL (raster or SVG)
  width: number;
  height: number;
  dx?: number; // Horizontal offset from node center
  dy?: number; // Vertical offset from node center
  position?: 'above' | 'below' | 'left' | 'right' | 'replace'; // Position relative to label if both are present
}
```
