...
| `getEquidistantPorts`  | `(shape: NodeShape, count?: number) => EquidistantPort[]`  | Compute N equidistant points along a shape's perimeter (by arc length, not angular spacing). Uses smart defaults per shape when `count` is omitted. |
