...
- `style?: { fill?: string; stroke?: string; strokeWidth?: number; opacity?: number; strokeDasharray?: string }` — base styling. `strokeDasharray` accepts presets (`'dashed'`, `'dotted'`, `'dash-dot'`, `'solid'`) or any SVG dasharray string.
