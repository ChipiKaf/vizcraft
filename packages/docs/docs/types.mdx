...
An `AnimationSpec` is what `builder.animate((anim) => ...)` compiles to, and what the player executes.
...
The property bag you pass to `anim.to(...)`.
...
Options you pass to `anim.to(props, opts)`.
...
You’ll typically use it via `anim.extendAdapter((adapter) => { adapter.register?.(...) })`.
