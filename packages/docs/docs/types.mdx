...
| `getEquidistantPorts`       | `(shape: NodeShape, count?: number) => EquidistantPort[]`  | Compute N equidistant points along a shape's perimeter (by arc length, not angular spacing). Delegates to a registered `PerimeterStrategy` or falls back to a bounding-box approximation. |
...
- `id: string` — stable identifier (e.g. `'p0'`, `'p1'`, …).
