...
An edge connecting two nodes — or a **dangling edge** with one (or both) endpoints at a free canvas coordinate. All edges are rendered as `<path>` elements.

**Key properties**

- `id: string` — unique id (commonly `"a->b"`).
- `from?: string` / `to?: string` — node ids. Both are optional: omit one to create a dangling edge.
- `fromAt?: Vec2` — free-endpoint coordinate for the source end (used when `from` is omitted).
- `toAt?: Vec2` — free-endpoint coordinate for the target end (used when `to` is omitted).
...
- `fromAngle?: number` — fixed perimeter angle (degrees, 0 = right, 90 = down) on the source node. Overrides port and boundary resolution.
- `toAngle?: number` — fixed perimeter angle (degrees, 0 = right, 90 = down) on the target node. Overrides port and boundary resolution.
...
| `findPort`                  | `(node: VizNode, portId: string) => NodePort \| undefined`       | Looks up a port by id in the node's effective ports. Also resolves legacy `p0`–`pN` IDs by index.                                                               |
| `findPortNearest`           | `(node: VizNode, x: number, y: number) => NodePort \| undefined` | Returns the port closest to the given point (in node-local coords). Uses the node's effective ports.                                                            |
...
| `getEquidistantPorts`       | `(shape: NodeShape, count?: number) => EquidistantPort[]`        | Compute N equidistant points along a shape's perimeter. Ports receive **location-based IDs** (side-based or angle-bucket) that are stable across count changes. |
| `toNodePorts`               | `(ports: EquidistantPort[]) => NodePort[]`                       | Convert equidistant ports to `NodePort[]` with `offset` and `direction`, ready for use on a `VizNode`.                                                          |
| `registerPerimeterStrategy` | `(strategy: PerimeterStrategy<K>) => void`                       | Register (or replace) a perimeter strategy for a shape kind. Use to add custom shape support or override a built-in.                                            |

### EquidistantPort {#equidistant-port}

Returned by `getEquidistantPorts`. Each entry represents a point on the shape's perimeter, equidistant by **arc length**.

**Shape**

- `id: string` — **location-based** identifier that encodes the port's approximate position on the perimeter (see [Port ID Schemes](#port-id-schemes) below).
- `angle: number` — angle from center in degrees (0 = right, 90 = down).
- `t: number` — parametric perimeter proportion in [0, 1).
- `x: number` — X offset from node center.
- `y: number` — Y offset from node center.

#### Port ID Schemes {#port-id-schemes}

Port IDs are **stable across count changes** — ports near the same perimeter region keep the same ID prefix regardless of the total port count.

**Side-based IDs** (polygon shapes with named sides):

| Shape           | Side labels                                                         | Example IDs                    |
| --------------- | ------------------------------------------------------------------- | ------------------------------ |
| `rect`          | `top`, `right`, `bottom`, `left`                                    | `top-0`, `right-1`, `bottom-0` |
| `diamond`       | `top-right`, `bottom-right`, `bottom-left`, `top-left`              | `top-right-0`, `bottom-left-0` |
| `triangle`      | depends on direction (e.g. `right`, `bottom`, `left` for `'up'`)    | `right-0`, `bottom-0`          |
| `hexagon`       | 6 directional labels (e.g. `top-right`, `right`, `bottom-right`, …) | `right-0`, `top-left-0`        |
| `parallelogram` | `top`, `right`, `bottom`, `left`                                    | `top-0`, `left-0`              |
| `trapezoid`     | `top`, `right`, `bottom`, `left`                                    | `top-0`, `bottom-1`            |

**Angle-bucket IDs** (curved and complex shapes):

| Shape               | Bucket size | Example IDs                     |
| ------------------- | ----------- | ------------------------------- |
| `circle`, `ellipse` | 90°         | `0-0`, `90-1`, `180-0`, `270-0` |
| `cylinder`          | 90°         | `0-0`, `270-1`                  |
| `star`, `cross`     | 90°         | `0-0`, `90-0`, `180-1`          |

:::tip Legacy compatibility
`findPort` still resolves the old `p0`–`pN` sequential IDs by treating the numeric suffix as a port index. This eases migration — existing `fromPort('p3')` references continue to work.
:::

### PerimeterStrategy\<K\> {#perimeter-strategy}

A pluggable strategy that teaches `getEquidistantPorts` how to compute equidistant ports for a given shape kind. Register via `registerPerimeterStrategy`.

**Shape**

- `kind: K` — the shape kind this strategy handles (e.g. `'circle'`, `'cylinder'`).
- `defaultCount: number | ((shape) => number)` — default port count when the caller omits `count`. Can be a fixed number or a function that derives the count from the shape (e.g. `star` → `points * 2`).
- `computePorts(shape, count): EquidistantPort[]` — compute equidistant ports for the given shape and count.

```ts
import { registerPerimeterStrategy } from 'vizcraft';
...
### Smart defaults per shape

| Shape           | Default count | Strategy                                            |
| --------------- | ------------- | --------------------------------------------------- |
| `rect`          | 8             | Polygon perimeter walk (corners + midpoints)        |
| `circle`        | 8             | Uniform angular spacing (≡ arc-length for circles)  |
| `ellipse`       | 8             | Numerical arc-length integration                    |
| `diamond`       | 4             | Polygon perimeter walk                              |
| `hexagon`       | 6             | Polygon perimeter walk                              |
| `triangle`      | 6             | Polygon perimeter walk                              |
| `parallelogram` | 8             | Polygon perimeter walk                              |
| `trapezoid`     | 8             | Polygon perimeter walk                              |
| `star`          | 2 × points    | Polygon perimeter walk                              |
| `cross`         | 12            | Polygon perimeter walk                              |
| `cylinder`      | 10            | Composite perimeter (ellipse arcs + straight sides) |
| All others      | 8             | Bounding-box rectangle fallback                     |
...
- `fromAngle? / toAngle?: number` — fixed perimeter angles (degrees, 0 = right, 90 = down). Overrides port and boundary resolution.
