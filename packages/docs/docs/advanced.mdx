...
const exportUiPlugin: VizPlugin = (builder) => {
  builder.on('mount', ({ container, controller }) => {
     ...
     btn.onclick = () => {
        // Logic to export `builder.svg()` to PNG.
        // For animated export (frame snapshots), use: builder.svg({ includeRuntime: true })
     };
...
By default, VizCraft computes edge endpoints by projecting from one node center toward the other and intersecting the shape boundary. This works well for most graphs, but sometimes you need an edge to leave or arrive at a specific angle — for example, a feedback loop that exits the bottom of a node and enters from the right.

## `.fromAngle(deg)` / `.toAngle(deg)`

Set a fixed angle (in degrees, where **0 = right**, **90 = down**, **180 = left**, **270 = up**) for the source or target endpoint. The edge endpoint is computed as the intersection of a ray at that angle with the node's shape perimeter.
...
viz()
  ...
  .at(300, 100)
  .circle(30)
  // Edge leaves node A from the bottom (90°) and enters node B from the top (270°)
  .edge('a', 'b')
  .fromAngle(90)
  .toAngle(270)
  .arrow()
...
You can also set angles via `EdgeOptions`:
...
viz()
  ...
  .at(300, 100)
  .circle(30)
  .edge('a', 'b', { fromAngle: 90, toAngle: 270, arrow: true })
  .build();
```

### Precedence

When `fromAngle` or `toAngle` is set, it takes precedence over `fromPort`/`toPort` and boundary auto-resolution. This lets you combine ports on one end with a fixed angle on the other:

```ts
.edge('a', 'b').fromPort('right').toAngle(180).arrow()
```

### Shape support

Every built-in shape supports angle-based anchoring. Shapes with exact mathematical boundaries (circle, ellipse, rect, diamond) produce precise results. Complex shapes (cloud, star, cross, etc.) use their bounding approximation — the same geometry used for standard boundary anchoring.

### `computeNodeAnchorAtAngle(node, angleDeg)`

For advanced use (e.g. custom renderers), you can call `computeNodeAnchorAtAngle` directly to resolve where a given angle intersects a node's shape perimeter:
...
`getEquidistantPorts(shape, count?)` computes **N points equally spaced by perimeter arc length** along any shape. This is useful for port-based editors, circuit diagrams, or any scenario where ports must be evenly distributed around a node.

The example below places 10 equidistant ports on a **cylinder** shape and connects edges to several of them:
...
// 1. Define the shape and compute 10 equidistant ports
const shape = { kind: 'cylinder', w: 120, h: 80 };
const ports = getEquidistantPorts(shape, 10);
const nodePorts = toNodePorts(ports);

// 2. Build the scene, adding ports via the builder
const b = viz().view(600, 350);
const db = b.node('db').at(300, 175).cylinder(120, 80)
.label('Database').fill('#DBEAFE').stroke('#1D4ED8', 2);
for (const p of nodePorts) db.port(p.id, p.offset, p.direction);

// 3. Add service nodes and connect to specific ports
b.node('svc1').at(80, 60).rect(80,40,6).label('Svc A').fill('#DCFCE7').stroke('#166534',2);
b.node('svc2').at(80, 280).rect(80,40,6).label('Svc B').fill('#FEF3C7').stroke('#92400E',2);
b.node('svc3').at(520, 60).rect(80,40,6).label('Svc C').fill('#FCE7F3').stroke('#9D174D',2);
b.node('svc4').at(520,280).rect(80,40,6).label('Svc D').fill('#E0E7FF').stroke('#3730A3',2);
...
const el = document.getElementById('viz-equidistant');
if (el) b.mount(el);
`}>
...
Each returned `EquidistantPort` includes:
...
Use `toNodePorts()` to convert the result into standard `NodePort[]` objects that you can assign directly to `node.ports`.

:::tip
When `count` is omitted, a shape-specific smart default is used (e.g. 8 for rect, 6 for hexagon, 4 for diamond). See [EquidistantPort](./types#equidistant-port) for the full reference.
:::
