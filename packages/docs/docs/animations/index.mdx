...
You can register custom properties directly on the `anim` builder via `anim.extendAdapter(...)` ([ExtendAdapter](../types.mdx#extendadapter-advanced)). Here’s an example that animates a circle node’s radius via a custom prop named `r`.
