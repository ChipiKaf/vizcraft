...
return defaultResolver(edge, scene);
    ...
    const bend = typeof metaObj.bend === 'number' ? metaObj.bend : 80;
    const cx = (from.pos.x + to.pos.x) / 2;
    const cy = Math.min(from.pos.y, to.pos.y) - bend;

    // Consumer-supplied SVG path `d` (quadratic curve)
...
.label('default', { dy: 12 })
.done()
// Same endpoints, but custom routing via `edge.meta`
.edge('a', 'b', 'e-custom')
.meta({ customRouting: true, bend: 90 })
.dashed()
.arrow()
.label('custom', { dy: -14 })
.done();
...
## Custom Edge Paths (Resolver Hook)

Use `builder.setEdgePathResolver()` to override how edge path geometry is computed.

The resolver receives the edge, the full scene, and a `defaultResolver` you can fall back to.
This works nicely with `edge.meta` as an opt-in flag.
...
const bend = typeof edge.meta?.bend === 'number' ? edge.meta.bend : 80;
const cx = (from.pos.x + to.pos.x) / 2;
const cy = Math.min(from.pos.y, to.pos.y) - bend;
...
builder
.node('a').at(100, 160).circle(18).label('A', { fontWeight: 600 }).done()
.node('b').at(460, 160).rect(70, 44, 10).label('B', { fontWeight: 600 }).done()

.edge('a', 'b', 'e-default').arrow().label('default', { dy: 12 }).done()
.edge('a', 'b', 'e-custom')
.meta({ customRouting: true, bend: 90 })
.dashed()
.arrow()
...
const container = document.getElementById('viz-edge-resolver');
if (container) builder.mount(container);
`}>

  <div style={{ height: '300px', width: '100%', position: 'relative' }}>
    <VizMount builder={edgeResolverScene} style={{ height: '100%', width: '100%' }} />
  </div>
</CodePreview>
