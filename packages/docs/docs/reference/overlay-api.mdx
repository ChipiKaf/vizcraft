...
`signal` follows the rendered edge path between `from` and `to` by default — including waypoints, curves, and orthogonal/boundary routing — and falls back to straight center-to-center interpolation when no edge connects the two nodes. It can park inside a node after arrival or move through a declarative multi-hop chain. See [SignalOverlayParams](./types#signaloverlayparams) and [SignalOverlayHop](./types#signaloverlayhop) for the exported types.

- Edge following is on by default; hops travelling against an edge's direction sample its path backwards. Set `followEdge: false` to force straight interpolation.
- With multiple parallel edges between the same nodes, the first declared edge wins; set `edgeId` to follow a specific one.
...
- If edge resolution fails (no matching edge, unknown `edgeId`), the renderer falls back to straight center interpolation before arrival.
...
| `color?`       | `string`             | CSS color for the ball fill; overrides the default class fill (`#3b82f6`)                                                               |
| `glowColor?`   | `string`             | Separate glow / halo color applied as a drop-shadow; defaults to `color` when omitted                                                   |
| `followEdge?`  | `boolean`            | Follow the matching edge path (default `true`); set `false` for straight center-to-center interpolation                                 |
