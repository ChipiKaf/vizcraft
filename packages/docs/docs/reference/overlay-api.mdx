...
| ------------- | --------------------------------------------------------- | ------------------------------------------------- |
| `signal`      | `{ from, to, progress, magnitude, followEdge?, edgeId? }` | Moving marker between nodes or along an edge path |
| `grid-labels` | `{ colLabels, rowLabels, xOffset, yOffset }`              | Grid axis labels                                  |
| `data-points` | varies                                                    | Points attached to nodes                          |

### `signal`

`signal` interpolates between node centers by default.
...
- If edge resolution fails or is ambiguous, the renderer falls back to straight center interpolation.

| Param         | Type      | Description                                                               |
| ------------- | --------- | ------------------------------------------------------------------------- |
| `from`        | `string`  | Source node id used for fallback interpolation and implicit edge matching |
| `to`          | `string`  | Target node id used for fallback interpolation and implicit edge matching |
| `progress`    | `number`  | Position along the motion path, typically `0..1`                          |
