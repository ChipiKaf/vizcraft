...
| `steps`       | `VizStepSpec[]`       | —        | Step-through walkthrough (future). See [VizStepSpec](#vizstepspec)     |
...
One step in a `VizSpec.steps` walkthrough. Silently ignored until the
step-controller feature is activated.

| Field         | Type                  | Description                                          |
| ------------- | --------------------- | ---------------------------------------------------- |
| `label`       | `string`              | Descriptive label for the step                       |
| `highlight`   | `string[]`            | Node ids to visually highlight                       |
| `overlays`    | `StaticOverlaySpec[]` | Overlays added on top of the base scene              |
| `signals`     | `AutoSignalSpec[]`    | Signals to animate when this step activates          |
| `autoAdvance` | `boolean`             | Auto-advance after all non-looping signals complete  |
