...
Before arrival, the signal uses the normal center-to-center or edge-following motion rules. When `resting` or `parkAt` are set and `progress >= 1`, the signal parks at the chosen node center plus any offsets.
...
| `progress`     | `number`  | Position along the motion path; parked placement begins at `>= 1`          |
