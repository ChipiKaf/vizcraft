...
| `ports?`      | `NodePort[]`               | Explicit connection ports                                                                       |
...
Computes the bounding-box size of any `NodeShape`.
...
Handles every `NodeShape` variant (circle, rect, diamond, ellipse, hexagon, star, trapezoid, etc.).
