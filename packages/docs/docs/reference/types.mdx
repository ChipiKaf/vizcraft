...
type RichTextToken =
  | {
      ...
      code?: boolean; href?: string; fill?: string;
      fontSize?: number | string; fontWeight?: number | string;
      fontFamily?: string; baselineShift?: 'sub' | 'super';
      className?: string;
    }
  | { kind: 'newline' };
```
...
```typescript
type VizPlugin<Options = any> = (builder: VizBuilder, options?: Options) => void;
```
...
*Found a problem? [Open an issue](https://github.com/ChipiKaf/vizcraft/issues/new?labels=documentation&title=Docs:+Types) on GitHub.*
