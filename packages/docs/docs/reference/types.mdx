...
Compartments are auto-sized when no explicit height is given — the builder estimates height from the label's line count and font size.
...
type TooltipContent =
  | string
  | { title?: string; sections: TooltipSection[] };
```
