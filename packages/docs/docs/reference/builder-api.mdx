...
:::tip Two styles
Every node and edge can be created with **fluent chaining** or a **declarative options object**. See [Fluent vs Declarative API](/explanations/fluent-vs-declarative) for guidance on when to use each.
:::
...
Applies an auto-layout algorithm to compute node positions.
