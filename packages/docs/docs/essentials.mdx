...
import { viz, circularLayout, gridLayout, registerIcon, getEquidistantPorts, toNodePorts } from 'vizcraft';

export const _registerEssentialsIcons = (() => {
  registerIcon(
    'custom-check',
...
| `image`                        | `{ href, w, h, …opts }`                        | `.image(href, w, h, opts)`        |
...
| `fromAngle` / `toAngle`     | `number`                                       | `.fromAngle(deg)` / `.toAngle(deg)`       |
| `label`                     | `string \| labelObj \| labelObj[]`             | `.label(text, opts)`                      |
| `hitArea`                   | `number`                                       | `.hitArea(width)`                         |
| `meta`                      | `Record<string, unknown>`                      | `.meta(value)`                            |
| `data`                      | `unknown`                                      | `.data(value)`                            |
...
You can embed images (raster or SVG) directly inside nodes using `.image()`. You can specify dimensions, positioning (`center`, `above`, `below`, `left`, `right`), and `dx`/`dy` offsets.
...
// Image centered in the shape
.node('n3').at(500, 75).diamond(80, 80)
.image('/dollar-icon.svg', 30, 30, { position: 'center' })
.label('Centered', { dy: 30 })
.done();
...
### Embedded Icons & Inline SVG

If you want icons without managing image URLs, use `.icon()` with the icon registry.

- Built-in icons are available by id (e.g. `'database'`, `'server'`, `'cloud'`, `'person'`).
- Register your own via `registerIcon(id, svgString)`.

For fully custom markup, use `.svgContent(...)` to embed inline SVG.
...
const builder = viz().view(760, 170);
...
.rect(170, 70, 10)
.fill('#a6e3a1')
.image('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="4" fill="black"/><circle cx="12" cy="12" r="4" fill="white"/></svg>', 28, 28, { position: 'left', dx: -6 })
.label('image()', { dx: 16 })
...
.rect(170, 70, 10)
.fill('#f9e2af')
.svgContent('<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 3l3 6 6 1-4.5 4.3L17.8 21 12 17.9 6.2 21l1.3-6.7L3 10l6-1 3-6z" fill="currentColor"/></svg>', { w: 28, h: 28, position: 'center' })
.label('svgContent()', { dy: 30 })
.done();

const container = document.getElementById('viz-embedded-media');
if (container) builder.mount(container);
`}>
...
### Node Border Styles (Dash Patterns)

Nodes support dashed, dotted, and custom border styles via \`.dashed()\`, \`.dotted()\`, or \`.dash(pattern)\`.
This works the same way as [edge dash patterns](#dash-patterns) — preset names resolve to standard SVG \`stroke-dasharray\` values.
...
const el = document.getElementById('viz-node-dash');
if (el) builder.mount(el);
`}>
...
You can also set dash patterns via the `style` property or the declarative `dash` option:

```ts
// via style object
viz().node('boundary').rect(200, 120).stroke('#6c7086').dash('dashed');

// via declarative options
viz().node('boundary', {
  rect: { w: 200, h: 120 },
  stroke: '#6c7086',
  dash: 'dashed',
...
Add a drop shadow behind any node shape with `.shadow()`. Call it with no arguments for a sensible default,
or pass a config object to customize offset, blur, and color.

| Property | Type     | Default             | Description                         |
| -------- | -------- | ------------------- | ----------------------------------- |
| `dx`     | `number` | `2`                 | Horizontal offset                   |
| `dy`     | `number` | `2`                 | Vertical offset                     |
| `blur`   | `number` | `4`                 | Gaussian blur radius (stdDeviation) |
| `color`  | `string` | `'rgba(0,0,0,0.2)'` | Shadow color                        |

Shadows are rendered via SVG `<filter>` elements in `<defs>`.
When multiple nodes share the same shadow config, a single filter definition is reused automatically.
...
const el = document.getElementById('viz-node-shadow');
if (el) builder.mount(el);
`}>
...
You can also set shadows via the declarative `shadow` option:

```ts
// shorthand — applies default shadow
viz().node('card', {
  rect: { w: 180, h: 100 },
  fill: '#ffffff',
  stroke: '#e0e0e0',
...
Give nodes and edges a hand-drawn, sketchy appearance with `.sketch()`.
The effect uses an SVG `feTurbulence` + `feDisplacementMap` filter for wobbly strokes, plus round line caps and joins.

You can apply it **per-node**, **per-edge**, or **globally** on the builder.

| Scope    | API                               | Description                              |
| -------- | --------------------------------- | ---------------------------------------- |
| Per-node | `.sketch()` / `.sketch({ seed })` | Sketchy rendering for a single node      |
| Per-edge | `.sketch()`                       | Sketchy rendering for a single edge      |
| Global   | `viz().sketch()`                  | Apply sketch mode to all nodes and edges |

Pass a `seed` number for deterministic jitter — the same seed produces the same wobble across renders.
Without a seed, VizCraft derives one from the element id.
...
.sketch() // ← global sketch mode: nodes + edges
.node('a').at(110, 100).rect(130, 60, 8)
.fill('#ffffff').stroke('#333').label('Node A')
.node('b').at(310, 100).rect(130, 60, 8)
.fill('#ffffff').stroke('#333').label('Node B')
.edge('a', 'b').arrow()
.done();

const el = document.getElementById('viz-node-sketch');
if (el) builder.mount(el);
`}>
...
Global sketch mode applies the effect to every node and edge:

```ts
// All shapes and edges get the sketchy effect
viz()
  .sketch()
  .node('a')
  .at(100, 100)
...
// Global sketch with a fixed seed for deterministic output
viz().sketch(true, 42).node('a').at(100, 100).rect(120, 60).done();
```

You can also use the declarative API:

```ts
viz().node('card', {
  rect: { w: 180, h: 100 },
  sketch: true, // default seed derived from id
});

viz().node('card', {
  rect: { w: 180, h: 100 },
  sketch: { seed: 7 }, // explicit seed
});
...
| `fontFamily`    | \`string\`                            | Custom font family for the label (e.g. \`'Georgia'\`, \`'Comic Sans MS'\`).            |
| `fontSize`      | \`number \| string\`                  | Font size for the label (e.g. \`14\` or \`'1.2em'\`).                                  |
...
Use `.richLabel((l) => ...)` on both nodes and edges to create **mixed-format** labels (bold/italic/code/links + sub/sup) rendered as nested SVG `<tspan>` spans.
...
const builder = viz().view(680, 240);
...
const el = document.getElementById('viz-rich-text-labels');
if (el) builder.mount(el);
`}>

  <VizMount
    builder={richTextLabelScene}
    style={{ height: '240px', width: '100%' }}
  />
</CodePreview>

:::note
Rich text labels currently don’t auto-wrap to a `maxWidth`. Use `.newline()` to control line breaks.
:::
...
When you need **N evenly spaced ports** along a shape's perimeter (think circuit-board connectors, port-based editors, or radial menus), use `getEquidistantPorts`.

Ports receive **stable, location-based IDs** so that persisted edge–port references survive count changes.

export const equidistantPortScene = (() => {
  const rectPorts = toNodePorts(getEquidistantPorts({ kind: 'rect', w: 120, h: 60 }, 8));
  const circlePorts = toNodePorts(getEquidistantPorts({ kind: 'circle', r: 40 }, 8));
  const hexPorts = toNodePorts(getEquidistantPorts({ kind: 'hexagon', r: 40 }, 6));
  const b = viz().view(700, 320);
  const nb = b.node('rect').at(120, 160).rect(120, 60, 6).label('Rect').fill('#DBEAFE').stroke('#1D4ED8', 2);
  for (const p of rectPorts) nb.port(p.id, p.offset, p.direction);
  const nb2 = b.node('circle').at(360, 160).circle(40).label('Circle').fill('#FEF3C7').stroke('#92400E', 2);
  for (const p of circlePorts) nb2.port(p.id, p.offset, p.direction);
  const nb3 = b.node('hex').at(580, 160).hexagon(40).label('Hex').fill('#DCFCE7').stroke('#166534', 2);
  for (const p of hexPorts) nb3.port(p.id, p.offset, p.direction);
  b.edge('rect', 'circle').fromPort('right-0').toPort('180-0').arrow().stroke('#6366F1').label('right-0 → 180-0');
  b.edge('circle', 'hex').fromPort('0-0').toPort('left-0').arrow().stroke('#059669').label('0-0 → left-0');
  b.edge('rect', 'hex').fromPort('top-1').toPort('top-left-0').arrow().stroke('#DC2626').label('top-1 → top-left-0');
  return b.done();
})();
...
// Generate equidistant ports with stable, location-based IDs
const rectPorts = toNodePorts(getEquidistantPorts({ kind: 'rect', w: 120, h: 60 }, 8));
// → [{ id: 'top-0', … }, { id: 'top-1', … }, { id: 'right-0', … }, …]

const circlePorts = toNodePorts(getEquidistantPorts({ kind: 'circle', r: 40 }, 8));
// → [{ id: '0-0', … }, { id: '0-1', … }, { id: '90-0', … }, …]

const hexPorts = toNodePorts(getEquidistantPorts({ kind: 'hexagon', r: 40 }, 6));
// → [{ id: 'top-right-0', … }, { id: 'right-0', … }, …]

const b = viz().view(700, 320);

// Attach generated ports to each node
const rect = b.node('rect').at(120, 160).rect(120, 60, 6)
  .label('Rect').fill('#DBEAFE').stroke('#1D4ED8', 2);
for (const p of rectPorts) rect.port(p.id, p.offset, p.direction);

const circle = b.node('circle').at(360, 160).circle(40)
  .label('Circle').fill('#FEF3C7').stroke('#92400E', 2);
for (const p of circlePorts) circle.port(p.id, p.offset, p.direction);

const hex = b.node('hex').at(580, 160).hexagon(40)
  .label('Hex').fill('#DCFCE7').stroke('#166534', 2);
for (const p of hexPorts) hex.port(p.id, p.offset, p.direction);

// Connect using stable port IDs — hover nodes to see ports
b.edge('rect', 'circle').fromPort('right-0').toPort('180-0').arrow();
b.edge('circle', 'hex').fromPort('0-0').toPort('left-0').arrow();
b.edge('rect', 'hex').fromPort('top-1').toPort('top-left-0').arrow();
b.done();
`}>

  <VizMount builder={equidistantPortScene} style={{ height: '320px', width: '100%' }} />
</CodePreview>

Hover over any node to see its port positions. Edge labels show the port IDs used for each connection — notice how they encode the port's location (e.g. `right-0`, `180-0`, `top-left-0`).

```typescript
import { getEquidistantPorts, toNodePorts, findPortNearest } from 'vizcraft';

// 8 equidistant ports on a rectangle — location-based IDs
const ports = getEquidistantPorts({ kind: 'rect', w: 120, h: 60 });
// → [{ id: 'top-0', … }, { id: 'top-1', … }, { id: 'right-0', … }, … ]

// Custom count: 12 ports on a circle — angle-bucket IDs
const circlePorts = getEquidistantPorts({ kind: 'circle', r: 40 }, 12);
// → [{ id: '0-0', … }, { id: '0-1', … }, { id: '0-2', … }, { id: '90-0', … }, … ]

// Convert to NodePort[] for use on a VizNode
...
if (nearest) edgeBuilder.toPort(nearest.id);
```

Unlike simple angular spacing, `getEquidistantPorts` distributes points by **perimeter arc length** — so ports on a tall rectangle are truly evenly spaced around the edges, not bunched at corners.

Port IDs are **stable across count changes**: ports near the same perimeter region keep the same ID prefix regardless of the total port count. This means persisted `fromPort`/`toPort` references survive when the port count changes.

| Field    | Description                                                                  |
| -------- | ---------------------------------------------------------------------------- |
| `id`     | Location-based identifier (e.g. `top-0`, `right-1`, `0-0`, `270-1`)         |
| `angle`  | Angle from center in degrees (0 = right, 90 = down)                         |
| `t`      | Parametric proportion along perimeter \[0, 1)                               |
| `x`, `y` | Offset from node center                                                     |

**ID schemes:**
- **Polygon shapes** with named sides (rect, diamond, triangle, hexagon, parallelogram, trapezoid): `{side}-{index}` (e.g. `top-0`, `right-1`)
- **Curved / complex shapes** (circle, ellipse, cylinder, star, cross): `{angleBucket}-{index}` where the bucket is a 90° quadrant (e.g. `0-0`, `90-1`, `270-0`)

:::tip Legacy compatibility
`findPort` still resolves old `p0`–`pN` sequential IDs by treating the numeric suffix as a port index. Existing `fromPort('p3')` references continue to work.
:::

:::tip
See [EquidistantPort](./types#equidistant-port) for the full type reference, [port ID schemes](./types#port-id-schemes), and smart-default counts per shape.
:::

### Dangling Edges

A **dangling edge** has one (or both) endpoints at an arbitrary canvas coordinate rather than attached to a node. This is essential for interactive editors — for example, while a user drags to connect two nodes, the "in-progress" edge starts at a port and follows the cursor.

#### Mental model

- Omit `from` and supply `fromAt: { x, y }` to place the source end at a free coordinate.
- Omit `to` and supply `toAt: { x, y }` to place the target end at a free coordinate.
- At least one endpoint (`from`/`fromAt` or `to`/`toAt`) must be specified.
- Once the user completes the connection, convert it to a regular edge by setting `from`/`to` and clearing `fromAt`/`toAt`.
...
// A dangling edge — source is a node, target is a free coordinate
.danglingEdge('drag-preview')
.from('src')
.toAt({ x: 400, y: 100 })
.arrow()
...
const el = document.getElementById('viz-dangling');
if (el) builder.mount(el);
`}>
...
| Builder method            | Effect                                                        |
| ------------------------- | ------------------------------------------------------------- |
| `.danglingEdge(id)`       | Start defining a dangling edge. Returns an `EdgeBuilder`.     |
| `.danglingEdge(id, opts)` | Declarative form. Returns the parent `VizBuilder`.            |
| `.from(nodeId)`           | Attach the source end to an existing node (by id).            |
| `.to(nodeId)`             | Attach the target end to an existing node (by id).            |
| `.fromAt(pos)`            | Set the free-endpoint coordinate for the source end (`Vec2`). |
| `.toAt(pos)`              | Set the free-endpoint coordinate for the target end (`Vec2`). |

:::tip
Dangling edges work with all edge features — routing, markers, labels, styling, and animations.
:::

### Perimeter Anchors (fromAngle / toAngle)

By default, edge endpoints are computed by projecting from one node center toward the other. **Perimeter anchors** let you pin an edge to a specific angle on the node's shape boundary — useful for feedback loops, bus-style wiring, or diagrams where you need precise visual control over where edges attach.

The angle is in **degrees**: 0 = right, 90 = down, 180 = left, 270 = up.
...
builder
.node('server').at(130, 125).rect(110, 55, 6)
.label('Server').fill('#89b4fa')
.node('db').at(370, 125).cylinder(90, 55)
.label('Database').fill('#a6e3a1')
// Edge leaves server at 315° (upper-right) and enters db at 200° (lower-left)
.edge('server', 'db')
.fromAngle(315).toAngle(200).arrow().label('query').curved()
// Return edge: exits db at 160° and enters server at 225° (lower-left)
.edge('db', 'server')
.fromAngle(160).toAngle(225).arrow().label('result').curved()
.done();

const el = document.getElementById('viz-perimeter');
if (el) builder.mount(el);
`}>
...
When `fromAngle` or `toAngle` is set, it takes precedence over `fromPort`/`toPort` and the default boundary projection. You can mix them freely — for example, use a port on one end and an angle on the other.
