...
VizCraft supports seventeen built-in shapes plus a generic custom-path escape hatch:
...
const builder = viz().view(700, 860);
...
`}>
  ...
  <VizMount
    builder={shapesScene}
    style={{ height: '820px', width: '100%', border: '1px solid #ccc' }}
  />
</CodePreview>
