...
export const equidistantPortScene = (() => {
  const rectPorts = toNodePorts(
    getEquidistantPorts({ kind: 'rect', w: 120, h: 60 }, 8)
  );
  const circlePorts = toNodePorts(
    getEquidistantPorts({ kind: 'circle', r: 40 }, 8)
  );
  const hexPorts = toNodePorts(
    getEquidistantPorts({ kind: 'hexagon', r: 40 }, 6)
  );
