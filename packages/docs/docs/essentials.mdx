...
Instead of chaining many fluent methods, you can pass an **options object** as the second argument to `.node()` or the third argument to `.edge()`. This configures the element in a single call and returns the `VizBuilder` so you can continue adding more elements.

```typescript
// Fluent style
viz()
  .node('a').at(100, 100).rect(80, 40).fill('steelblue').label('A')
  .edge('a', 'b').arrow().stroke('red')
  .build();

// Declarative style — same result
viz()
  .node('a', { at: { x: 100, y: 100 }, rect: { w: 80, h: 40 }, fill: 'steelblue', label: 'A' })
