...
// 8 equidistant ports on a rectangle (smart default)
const ports = getEquidistantPorts({ kind: 'rect', w: 120, h: 60 });
// → [{ id: 'p0', angle, t, x, y }, ... ]

// Custom count: 12 ports on a circle
...
Unlike simple angular spacing, `getEquidistantPorts` distributes points by **perimeter arc length** — so ports on a tall rectangle are truly evenly spaced around the edges, not bunched at corners. Each port includes:
...
:::tip
See [EquidistantPort](./types#equidistant-port) for the full type reference and smart-default counts per shape.
:::
